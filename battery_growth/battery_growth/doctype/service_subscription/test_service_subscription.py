...
class TestServiceSubscription(FrappeTestCase):
    ...
    def test_churn_date_cannot_precede_activation(self):
        with self.assertRaises(frappe.ValidationError):
            make_subscription(service_status="已流失", churn_date="2025-12-31", churn_reason="价格").insert()
    ...
    def test_other_churn_reason_requires_note(self):
        with self.assertRaises(frappe.ValidationError):
            make_subscription(service_status="已流失", churn_date="2026-02-01", churn_reason="其他").insert()
