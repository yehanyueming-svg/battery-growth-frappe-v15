...
class TestGrowthMetrics(FrappeTestCase):
	...
	def test_distributions_have_shares_top_ten_and_stable_ties(self):
		...
		result = self.get_metrics()
		self.assertEqual(result["distributions"]["regions"][0], {"label": "浙江省", "value": 12, "share": 80.0})
		self.assertEqual(result["distributions"]["plans"], [
			{"label": "畅换", "value": 12, "share": 80.0},
			{"label": "基础换电", "value": 2, "share": 13.33},
			{"label": "企业车队", "value": 1, "share": 6.67},
		])
		self.assertEqual(result["distributions"]["churn_reasons"], [{"label": "价格", "value": 1, "share": 100.0}])
		stations = result["distributions"]["stations"]
		self.assertEqual(len(stations), 10)
		self.assertEqual([entry["label"] for entry in stations[1:]], sorted(entry["label"] for entry in stations[1:]))
