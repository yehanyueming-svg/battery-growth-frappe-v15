...
def _load_dashboard_without_frappe():
    ...
    utils.getdate = (
        lambda value: value if isinstance(value, datetime.date) else datetime.date.fromisoformat(str(value))
    )
...
class TestDashboardAPI(FrappeTestCase):
    ...
    def test_invalid_filters_are_actionable_frappe_validation_errors(self):
        dashboard.normalize_filters = lambda _filters: (_ for _ in ()).throw(
            ValueError("开始日期不能晚于结束日期")
        )
    ...
    def test_operations_brief_is_post_whitelisted_and_denies_before_service_work(self):
        ...
        self.assertEqual(
            dashboard.frappe.allowed_http_methods_for_whitelisted_func[dashboard.generate_operations_brief],
            ["POST"],
        )
        dashboard.frappe.has_permission = lambda *_args, **_kwargs: False
        dashboard.normalize_filters = lambda _filters: self.fail("normalization must not run")
    ...
    def test_operations_brief_normalizes_once_and_parses_force_with_cint(self):
        brief_calls = []
        dashboard.get_operations_brief = lambda filters, force=False: brief_calls.append(
            (filters, force)
        ) or {"source": "rules"}
        self.assertEqual(
            dashboard.generate_operations_brief({"city": "杭州市"}, force="0"), {"source": "rules"}
        )
        self.assertEqual(
            dashboard.generate_operations_brief({"city": "杭州市"}, force="1"), {"source": "rules"}
        )
        self.assertEqual(
            brief_calls,
            [({"normalized": {"city": "杭州市"}}, False), ({"normalized": {"city": "杭州市"}}, True)],
        )
