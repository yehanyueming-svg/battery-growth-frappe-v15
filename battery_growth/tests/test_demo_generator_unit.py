...
def _load_generator_with_stub():
    ...
    try:
        ...
        for name, original_module in original_modules.items():
            if name in _STUB_BOUND_MODULES:
                continue
            if original_module is missing:
                sys.modules.pop(name, None)
            else:
...
class TestDemoGenerator(unittest.TestCase):
    ...
    def test_default_seed_has_zhejiang_majority_and_other_regions(self):
        ...
        self.assertGreater(
            sum(count for province, count in province_counts.items() if province != "浙江省"), 0
        )
        self.assertTrue(
    ...
    def test_seed_rejects_counts_outside_the_supported_range(self):
        for count in (0, 1001):
            with (
                ...
                self.assertRaisesRegex(ValidationError, "count 必须在 1 到 1000 之间"),
            ):
...
@unittest.skipIf(HAS_REAL_FRAPPE, "requires the no-Frappe fallback environment")
class TestNoFrappeIsolation(unittest.TestCase):
    def test_stub_loader_does_not_cache_bound_modules(self):
        """A later real-Frappe import must not reuse modules loaded against this stub."""
