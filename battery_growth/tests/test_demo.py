...
class TestDemoData(FrappeTestCase):
    ...
    def test_seed_contains_both_customer_types_and_churn(self):
        ...
        self.assertGreater(
            frappe.db.count("Service Subscription", {"is_mock": 1, "customer_type": "个人"}), 0
        )
        self.assertGreater(
            frappe.db.count("Service Subscription", {"is_mock": 1, "customer_type": "企业"}), 0
        )
        self.assertGreater(
            frappe.db.count("Service Subscription", {"is_mock": 1, "service_status": "已流失"}), 0
        )
