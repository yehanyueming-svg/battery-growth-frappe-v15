...
class TestWorkspaceStaticContract(unittest.TestCase):
    def test_workspace_metadata_and_navigation_contract(self):
        ...
        self.assertEqual({item["doc_view"] for item in workspace["shortcuts"]}, {"List", "New"})

        links = {
            (item["link_type"], item["link_to"]): item
    ...
    def test_workspace_introduces_no_custom_binary_assets(self):
        ...
        binary_assets = [
            ...
            if path.is_file()
            and path.suffix.lower() in {".gif", ".ico", ".jpeg", ".jpg", ".png", ".svg", ".webp"}
        ]
