...
def generate_rule_insights(metrics: dict) -> dict:
    ...
    if churn_rate >= 15:
        insights.append(
            _insight(
                "critical", "流失率需要立即关注", f"本期流失率为 {churn_rate:g}%", "优先核查高流失区域和套餐"
            )
        )
    elif churn_rate >= 8:
        insights.append(
            _insight("warning", "流失率偏高", f"本期流失率为 {churn_rate:g}%", "复盘流失原因并安排客户回访")
        )

    if net_growth < 0:
        insights.append(
            _insight("warning", "净增长为负", f"本期净增长 {net_growth:g} 户", "提高获客转化并减少可避免流失")
        )
    elif net_growth >= 10:
    ...
    if leading_share > 50:
        ...
        insights.append(
            _insight(
                "warning",
                "区域集中度偏高",
                f"{region} 占在服用户 {leading_share:g}%",
                "评估其他重点区域的拓展机会",
            )
        )
    ...
    if not insights:
        insights = [
            _insight(
                "info", "运营整体平稳", "当前未发现需要优先处理的聚合指标异常", "保持日常监测并按周期复盘"
            )
        ]
